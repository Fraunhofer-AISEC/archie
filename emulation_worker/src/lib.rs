...
mod logs;
use crate::logs::{State, Logs, MemInfo, Fault, FaultType, FaultModel};

mod hooks;
use crate::hooks::initialize_hooks;

#[pyfunction]
fn run_unicorn(pregoldenrun_data: &PyDict, faults: Vec<Fault>, config: &PyDict) -> PyResult<PyObject> {
    ...
    let py = gil.python();
    Ok(state_arc.logs.to_object(py))
}
