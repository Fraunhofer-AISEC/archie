...
use crate::{State, MemInfo, Fault, FaultType, FaultModel};

mod util;
use util::{log_tb_info, log_tb_exec, apply_model, undo_faults};
...
fn block_hook_cb(uc: &mut Unicorn<'_, ()>, address: u64, size: u32, state: &Arc<State>) {
    ...
    *last_tbid = address;
    ...
    if live_faults.len() == 0 && single_step_hook_handle.is_some() {
        uc.remove_hook(single_step_hook_handle.unwrap()).expect("failed removing single step hook");
    ...
    for fault in faults.values() {
        ...
        if fault.trigger.hitcounter == 1 && fault.trigger.address >= address && fault.trigger.address <= (address + size as u64) {
            let state_arc = Arc::clone(&state);
            let single_step_hook_closure = move |uc: &mut Unicorn<'_, ()>, address: u64, size: u32| {
                single_step_hook_cb(uc, address, size, &state_arc);
            };
            *single_step_hook_handle = Some(uc.add_code_hook(u64::MIN, u64::MAX, single_step_hook_closure).unwrap());
        }
    }
...
fn end_hook_cb(uc: &mut Unicorn<'_, ()>, address: u64, _size: u32, state: &Arc<State>, first_endpoint: u64) {
...
fn single_step_hook_cb(uc: &mut Unicorn<'_, ()>, address: u64, size: u32, state: &Arc<State>) {
    ...
    undo_faults(uc, *instruction_count, state.faults.read().unwrap(), state.live_faults.write().unwrap());
...
fn mem_hook_cb(uc: &mut Unicorn<'_, ()>, mem_type: MemType, address: u64, size: usize, _value: i64, state: &Arc<State>) -> bool {
    ...
    if let Some(mut element) = meminfo.get_mut(&(address, pc)) {
        ...
        meminfo.insert((address, pc), MemInfo{
            ins: address,
            counter: 1,
...
fn fault_hook_cb(uc: &mut Unicorn<'_, ()>, address: u64, _size: u32, state: &Arc<State>) {
    ...
    match fault.kind {
        FaultType::Data | FaultType::Instruction => {
            let fault_size = if matches!(fault.model, FaultModel::Overwrite) {fault.num_bytes} else { 1 };
            let data = BigUint::from_bytes_le(uc.mem_read_as_vec(fault.address, fault_size as usize).unwrap().as_slice());
            prefault_data = data.clone();
            println!("Overwriting {:?} with {:?}", data, apply_model(&data, fault));
            uc.mem_write(fault.address, apply_model(&data, fault).to_bytes_le().as_slice()).expect("failed writing fault data to memory");
        }
        FaultType::Register => {
            let register_value = BigUint::from(uc.reg_read(fault.address as i32).expect("failed reading from register"));
            prefault_data = register_value.clone();
            let new_value = apply_model(&register_value, fault);
            uc.reg_write(fault.address as i32, new_value.to_u64().unwrap()).expect("failed writing register fault");
        }
    }
    ...
    if fault.lifespan != 0 {
        ...
        live_faults.push((fault.trigger.address, prefault_data), u64::MAX - fault.lifespan as u64 + instruction_count);
    }
}

fn initialize_mem_hook(emu: &mut Unicorn<()>, state_arc: &Arc<State>) -> io::Result<()> {
    let state_arc = Arc::clone(&state_arc);
    let mem_hook_closure = move |uc: &mut Unicorn<'_, ()>, mem_type: MemType, address: u64, size: usize, value: i64| -> bool {
        mem_hook_cb(uc, mem_type, address, size, value, &state_arc)
    };
    emu.add_mem_hook(HookType::MEM_READ | HookType::MEM_WRITE, 0, u64::MAX, mem_hook_closure).expect("failed to add read mem hook");
...
fn initialize_block_hook(emu: &mut Unicorn<()>, state_arc: &Arc<State>) -> io::Result<()> {
    ...
    emu.add_block_hook(block_hook_closure).expect("failed to add block hook");

    Ok(())
}

fn initialize_end_hook(emu: &mut Unicorn<()>, state_arc: &Arc<State>, config: &PyDict, first_endpoint: u64) -> io::Result<()> {
    ...
    for obj in config_endpoints {
        ...
        emu.add_code_hook(address, address, end_hook_closure).expect("failed to add end hook");
    }

    Ok(())
}

fn initialize_fault_hook(emu: &mut Unicorn<()>, state_arc: &Arc<State>, faults: Vec<Fault>) -> io::Result<()> {
    for fault in faults {
        let state_arc = Arc::clone(&state_arc);
        ...
        emu.add_code_hook(fault.trigger.address, fault.trigger.address, fault_hook_closure).expect("failed to add fault hook");
    }

    Ok(())
}

pub fn initialize_hooks(emu: &mut Unicorn<()>, state_arc: &Arc<State>, faults: Vec<Fault>, config: &PyDict) -> io::Result<()> {
